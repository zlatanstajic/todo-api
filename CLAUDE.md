...
The full set of project rules — commands, the 100% coverage gate, the `UserScope` ownership invariant, the Controller → Service → Repository layering, error-response handling, the stub-based unit test style, the generated API documentation, and the translation, branch-naming, and generated-asset conventions — lives in [AGENTS.md](AGENTS.md). Read it before changing controllers, repositories, exceptions, or tests. It is the single source of truth for both files; add new rules there rather than duplicating them here.
...
- API docs are generated by Scramble from the code; there is no spec to hand-edit. Run `php artisan docs:build` to preview the static site CI deploys, and keep `dedoc/scramble` references out of `app/` — it is a dev dependency.
- Branch off `master` using kebab-case names (`issues/12-short-description`).
