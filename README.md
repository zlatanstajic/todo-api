...
The published documentation lives at **<https://zlatanstajic.github.io/todo-api/>**.
...
[`.github/workflows/docs.yml`](.github/workflows/docs.yml) runs `php artisan docs:build` on every push to `master` and publishes `build/docs` to GitHub Pages. It needs **Settings → Pages → Source: GitHub Actions** enabled once on the repository; the workflow can also be started by hand from the Actions tab.
