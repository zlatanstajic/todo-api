#!/usr/bin/env python3
"""Generate the 1200x630 Open Graph image for Todo API.
...
def render_logo(size):
    """Render a checked task box as a neon outline."""
    scale = 4
    ...
    mark_draw.rounded_rectangle(
        [unit(22), unit(22), unit(178), unit(178)],
        radius=unit(28),
        outline=INK + (255,),
        width=5 * scale,
    )
    mark_draw.line(
        [(unit(58), unit(102)), (unit(88), unit(132)), (unit(146), unit(66))],
        fill=INK + (255,),
        width=5 * scale,
        joint="curve",
    )
...
def main():
    ...
    tagline = "Get things done, one request at a time."
